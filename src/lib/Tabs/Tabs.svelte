<script lang="ts">
  ...
  function scroll(edge: 'start' | 'end'): void {
    ...
    // This file already had a reduced-motion @media block, and it was counted as
    // guarded on that alone -- but the block covers the indicator's CSS transition
    // and cannot touch a scrollBy `behavior`, which is not a style. Right row, wrong
    // reason: the JS scroll went on animating under the preference.
  ...
  function reconcileFocus(): void {
    ...
    const active = getActiveElement(scrollContainer);
    ...
    if (getActiveElement(target) !== target) {
      target.focus();
    }
  }
...
<style>
  ...
  .tabs-arrow:hover {
    color: var(--tabs-arrow-hover-color, var(--tabs-active-color, #0b57d0));
    background: var(
  ...
  .tabs-item.active {
    color: var(--tabs-active-color, #0b57d0);
    font-weight: var(--tabs-active-font-weight, 600);
