...
Neither dispatches a matching DOM event: `copy` and `error` are already `HTMLElement`'s
own native events, so `snippet.addEventListener('copy', ...)` or `.addEventListener('error', ...)`
registers without error but never runs for these callbacks -- use the JS properties above instead.
