...
Set `animateLegendValues` to route `.pie-legend-value` through `AnimatedNumber` — the same string the static legend already prints (`valueFormat` when supplied, else the built-in default, plus the percentage) is passed straight to it, so a slice's value rolls to its new figure instead of jumping when `data` changes. Off by default, and it only touches the `legendShowValues` list: the on-arc `<text>` labels can't host per-digit spans and are unaffected either way.

```svelte
<PieChart {data} showLegend legendShowValues animateLegendValues />
```
