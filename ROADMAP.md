...
`AttachmentChipRow`, `HITL`, and `TypewriterText` additionally have no entry in
`docs/_index.json` yet — worth a follow-up to document them, since the MCP server's
`get_component_docs` tool can't surface what isn't indexed. (`TaskList` is already
indexed there.)
