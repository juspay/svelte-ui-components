...
<script lang="ts">
  ...
  // 'sui-grid-item:onclick' / 'sui-grid-item:onkeydown' in ../dispatch.ts's
  // DISPATCH_COLLISION_EXCEPTIONS -- so dispatchEvents intentionally returns nothing
  // for either. They stay callback-only: `gridItem.onclick = fn` / `.onkeydown = fn`
...
<style>
  ...
  :host {
    display: var(--sui-grid-item-display, block);
  }
</style>
