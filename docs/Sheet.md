...
| `--sheet-panel-transition-duration`     | `300ms`                          | transform, opacity | Duration of the panel's slide/fade transition (both open and close share one token — unlike Modal, nothing here already has an in/out asymmetry to preserve). Read via `getComputedStyle`, not CSS cascading: checked in order, this token then `--motion-duration`, then `300ms`. |
| `--sheet-panel-transition-distance`     | `60px`                            | transform         | Distance (px) the panel travels from its edge (ignored when `side="center"`, which only fades). Checked in order: this token, then `--distance-overlay`, then `60px`.                                                                                                           |
| `--sheet-panel-transition-easing`       | `cubicOut`                        | transform, opacity | Easing curve of the panel transition, parsed from a CSS easing keyword or `cubic-bezier(x1, y1, x2, y2)`. Checked in order: this token, then `--ease-smooth-out`, then `--motion-easing`, then `cubicOut`.                                                                       |
...
- Under `prefers-reduced-motion: reduce`, the panel stops travelling from its edge (behaving like `side="center"`, opacity-only) and its transition duration is clamped to effectively instant — regardless of what `--sheet-panel-transition-duration` or `--motion-duration` resolve to. The clamp lives in the shared `tokenizedFly` helper, so a consumer-set duration token cannot re-enable motion for a user who has asked the OS to reduce it.
