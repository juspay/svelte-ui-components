...
| `--step-index-container-border`           | `none`                           | border              | Border of the step number/icon circle. No border renders by default — set this (directly, or via a `classes` override on the outer `.step`, which the custom property inherits into the circle) to add a themed ring around it. |
...
| `--step-text-font-weight`                 | `normal`                         | font-weight         | Weight of the step label text. Not set by default; use this instead of a `font-weight` declaration in a consumer's own component styles.                                                                                        |
