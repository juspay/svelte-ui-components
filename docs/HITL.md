...
| actions              | `HITLExtraAction[]`                       | No       | `-`                        | Extra side actions between cancel and confirm. Selecting one never settles the card (no completion state, no `onconfirm`) — it only pauses the countdown, like any other interaction, then calls the action's own `onSelect`. Each entry is `{ label, onSelect, testId?, classes?, ariaLabel? }`. |
| children             | `Snippet`                                 | No       | `-`                        | Arbitrary extra controls (free-text input, multi-select chips) rendered in the body above the action row, only while the card is pending.                                                                                                                                                         |
...
`title` maps to the `hITLTitle` element property (renamed because `title` is a reserved global HTML attribute); every other attribute name matches its `HITLProperties` field. `actions` and `children` are not wired into the custom element (see Notes above) — `sections`, `function-arguments`, `hidden-keys`, `initial-state` are set as object-valued properties, not attributes.
