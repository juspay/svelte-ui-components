...
| dataType             | `InputDataType`                                  | No       | `'text'`            | The native input type, and the switch `validateInput` runs on. **Only `text`, `tel`, `password` and `email` are validated** — `number`, `time`, `date`, `search` and `url` have no branch at all and are always reported Valid, so `onErrorMessage` never shows for them and the browser's own constraint validation is the only check; supply `validators` if you need more. `'tel'` strips non-digits and applies textTransformers; `'email'` uses RFC 5322 validation; `'password'` and `'text'` use pattern-based validation. See [Phone numbers](#phone-numbers) below. |
...
| required             | `boolean`                                        | No       | `-`                 | Appends a required asterisk beside the label and sets both `aria-required` and the native `required` attribute, enabling native form validation alongside assistive-technology support. The platform's own name, and the spelling every other form control in this library uses. Wins over `mandatory` when both are supplied.                                                                                                                                                                                                                                               |
| mandatory            | `boolean`                                        | No       | `false`             | **Deprecated, use `required`.** The original spelling of the same concept. It keeps working unchanged and is only consulted when `required` is not supplied. The `--input-mandatory-*` tokens and the `.input-mandatory-asterisk` class keep their names.                                                                                                                                                                                                                                                                                                                    |
...
`dataType="tel"` with no `validationPattern` checks only the characters a phone number
may contain — digits, spaces, `+`, `-`, `.`, `()` — and at most the 15 digits E.164
allows. It asserts nothing about any national format, so `+33 6 12 34 56 78`,
`(555) 123-4567` and `9876543210` are all accepted, and `555-CALL-NOW` is not.

**This changed.** `tel` previously required ten digits beginning 6-9 — an Indian mobile
number — unconditionally, so a consumer in any other market had correct input silently
marked Invalid by a rule they never chose. That rule is still shipped, as a preset you
ask for:

```svelte
<script>
  import { Input, TEL_PRESET_IN_MOBILE } from '@juspay/svelte-ui-components';
</script>
...
<Input
  ...
  onErrorMessage="Enter a 10-digit Indian mobile number"
/>
```

Calling the validator directly takes the preset as an option instead:

```typescript
import { validateInput, TEL_PRESET_IN_MOBILE } from '@juspay/svelte-ui-components';

validateInput(value, 'tel', null, null, [], { telPreset: TEL_PRESET_IN_MOBILE });
```

`TEL_PRESETS` holds every preset the library ships, keyed by name (`'in-mobile'`).
...
Only `text`, `tel`, `password` and `email` are validated by `validateInput`. The other
five are rendered and submitted normally and are never reported Invalid by it.
...
| ------------ | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `left-icon`  | `leftIcon`      | Icon inside the field on the leading edge. Nothing renders when the slot is empty, and the field keeps its normal padding — there is no built-in icon.  |
| `right-icon` | `rightIcon`     | Icon inside the field on the trailing edge. Nothing renders when the slot is empty, and the field keeps its normal padding — there is no built-in icon. |

The element claims the snippet only when the slot is actually filled, which matters here beyond
the glyph: supplying `leftIcon`/`rightIcon` is also what adds
`calc(var(--input-icon-size, 20px) + var(--input-icon-gap, 12px) * 2)` of padding to the control,
so an always-claimed snippet would indent every field around an empty box. For the same reason a
JavaScript-assigned `leftIcon`/`rightIcon` property keeps working when the slot is empty, while
slotted markup wins when it is not.

`onlefticonclick`/`onrighticonclick` apply to slotted content too: with a handler set, the icon
becomes a real `<button>` wrapping whatever the slot projects.

### Web Component Events

`onfocusout`, `onstatechange`, `onlefticonclick`, and `onrighticonclick` are available as JS
properties, and each also dispatches a same-named DOM custom event (bubbles, composed) for a
consumer who only calls `addEventListener` — each carries its own single argument as detail
(`focusout`'s is the `FocusEvent`, `statechange`'s is the `ValidationState`, and the two icon
click events carry no argument, since neither callback takes one):

```js
const input = document.querySelector('sui-input');
input.addEventListener('focusout', (e) => console.log(e.detail.target));
input.addEventListener('statechange', (e) => console.log(e.detail));
```

`onfocus`, `onblur`, `oninput`, `onpaste`, `onclick`, and `onkeydown` are JS-property callbacks
only and do not dispatch DOM events: all six names are already native `HTMLElement`/form
handlers, so a listener for any of them already sees the real event that bubbles out of the
shadow root on its own, and a synthetic duplicate under the same name would double-deliver it.
