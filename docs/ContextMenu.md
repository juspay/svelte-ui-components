...
| `--context-menu-panel-transition-duration`          | `150ms`                                          | transform, opacity | Duration of the dropdown panel's entrance/exit transition. Read via `getComputedStyle`, checked in order: this token, then `--duration-quick`, then `--motion-duration`, then `150ms`. |
| `--context-menu-panel-transition-distance`          | `12px`                                           | transform        | Distance (px) the dropdown panel travels on entrance/exit. Checked in order: this token, then `--distance-medium`, then `12px`.                                                        |
| `--context-menu-panel-transition-easing`            | `cubicOut`                                       | transform, opacity | Easing curve of the transition, parsed from a CSS easing keyword or `cubic-bezier(...)`. Checked in order: this token, then `--ease-smooth-out`, then `--motion-easing`, then `cubicOut`. |
