...
export function renderMarkdown(markdown: string, options: RenderMarkdownOptions = {}): string {
  /* Unwrapping happens before parsing, so the sanitizer sees the document the
     author meant rather than a fence around it. */
  ...
  const parsed = parseWith(source, options, rawHtml);
  if (parsed === null) {
    return '';
  }
  ...
  if (rawHtml === 'passthrough') {
    ...
    output = sanitized ?? parseWith(source, options, 'escape') ?? '';
  }
