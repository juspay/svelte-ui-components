<script lang="ts">
  ...
  /*
   * `matchMedia` builds a fresh `MediaQueryList` on every call and `draw` runs
   * on every animation frame, so reading the preference inside `draw` scaled
   * that work with frame rate. It is read once and kept current by the query's
   * own `change` event instead; `draw` and the two motion resolvers below see
   * one consistent value for the whole frame either way.
   */
