...
| usePortal    | `boolean`                                                  | No       | `false`     | When true, mounts the tooltip bubble directly on the root of the tree the trigger lives in — `document.body` normally, or the shadow root inside `<sui-tooltip>` — using `position: fixed` coordinates. Prevents clipping inside `overflow: hidden` or stacking-context ancestors (e.g. toolbar items). The bubble's own box is styled inline and survives either target, but the `--tooltip-*` custom properties it reads inherit from the shadow host and a `content` snippet carries scoping classes whose stylesheet lives in the shadow root, so both are lost on `document.body` and kept on the root. |
...
| `use-portal` | `usePortal` | `Boolean` | `false` | When present, mounts the bubble on this element's own shadow root with `position: fixed`. Prevents clipping inside `overflow: hidden` ancestors while keeping the bubble inside the stylesheet and token scope that theme it. |
