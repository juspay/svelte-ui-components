...
| inputProperties        | `OptionalInputProperties`                  | Yes      | `-`     | Pass-through configuration for the internal Input component (`placeholder`, `dataType`, `label`, `validators`, `readonly`, `spellcheck`, `inputMode`, and more) — see Input's own docs for the full, current list.                                                                 |
| testId                 | `string`                                   | No       | `-`     | `data-pw`/`testID` on the root `.container` element.                                                                                                                                                                                                                               |
...
| `--input-label-msg-text-color`          | `#4d6174`                                            | color                           |                                                                                               |
...
| `--input-error-msg-text-color`          | `#c5120a`                                            | color                           | Shared with `Input`'s own error text (6.06:1 on white, AA).                                   |
...
| `--input-info-msg-text-color`           | `#52525b`                                            | color                           | Neutral, not the error color — shared with `Input`'s own helper text.                         |
| `--input-btn-info-msg-margin`           | `12px 0px 0px 0px`                                   | margin                          | Margin around the info message.                                                               |
| `--inputbutton-mandatory-color`         | `#e11900`                                            | color                           | Color of the mandatory asterisk marker rendered next to the label.                            |
| `--inputbutton-sm-height`               | `36px`                                               | --input-height                  | Container height when `size="sm"`.                                                            |
| `--inputbutton-sm-padding`              | `6px 12px`                                           | --input-padding                 | Container padding when `size="sm"`.                                                           |
| `--inputbutton-md-height`               | `44px`                                               | --input-height                  | Container height when `size="md"`.                                                            |
| `--inputbutton-md-padding`              | `10px 16px`                                          | --input-padding                 | Container padding when `size="md"`.                                                           |
| `--inputbutton-lg-height`               | `54px`                                               | --input-height                  | Container height when `size="lg"`.                                                            |
| `--inputbutton-lg-padding`              | `14px 20px`                                          | --input-padding                 | Container padding when `size="lg"`.                                                           |
| `--inputbutton-external-error-color`    | `#c5120a`                                            | color                           | Color of the external error message (set via the `error` prop); 6.06:1 on white, AA.          |
| `--inputbutton-external-error-margin`   | `6px 0px 0px 0px`                                    | margin                          | Margin around the external error message.                                                     |
| `--input-button-font-weight`            | `500`                                                | font-weight                     | Font weight of the input-button container row.                                                |
...
```typescript
type InputButtonSize = 'sm' | 'md' | 'lg';
```

Preset size variants. `sm` renders at 36px height, `md` at 44px, `lg` at 54px. Each can be overridden via the corresponding `--inputbutton-{size}-height` and `--inputbutton-{size}-padding` CSS variables.

### OptionalInputProperties

`inputProperties` accepts the same optional props as the standalone `Input` component — see `Input`'s own documentation for the full, current list rather than a duplicated snapshot here (a prior inline copy of this type drifted out of date as `Input` grew new props).
