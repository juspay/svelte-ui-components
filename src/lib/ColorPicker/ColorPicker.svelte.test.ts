 ...
 *  a real click sequence fires pointerdown first, but `element.click()` only
...
describe('ColorPicker popover Escape handling', () => {
  it('closes the popover and returns focus to the trigger on Escape', async () => {
    ...
    // The shared dismissal registry listens on `document`, not `window` (see
    // src/lib/_interaction/dismissal.ts) -- capture-phase, so it sees the event
    // on its way down to whatever's focused. `window` has no such listener.
