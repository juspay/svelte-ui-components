...
| `--color-picker-input-height`                | `38px`                                                             | height         | Height of the inline hex value input in the trigger row; lower it for a compact boxed control. |
| `--color-picker-value-text-transform`        | `none`                                                             | text-transform | Text transform for the hex value in the inline and popover HEX inputs (e.g. `uppercase`).      |
