...
> **Note:** `onprogress` is a callback prop, not a DOM event — assign it as a plain
> property (`el.onprogress = (progress) => ...`). The custom element does not dispatch a
> matching DOM event: `progress` is already `HTMLElement`'s own native event, so
> `el.addEventListener('progress', ...)` registers without error but this component's
> handler is never called through it.

> **Svelte-only:** `renderCharacter` (receives `TypewriterCharacterContext`) takes argument, so it cannot be expressed as a named slot: a Web Component `<slot>` projects markup, it does not forward Svelte snippet parameters, so the argument above would be silently dropped. Use the Svelte component directly when you need this.
