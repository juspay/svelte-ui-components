...
`onchange` is a callback prop, not a DOM event — assign it as a plain property
(`el.onchange = (index, label) => ...`). The custom element does not dispatch a matching
DOM event, so `el.addEventListener('change', ...)` registers without error but the
handler is never called.

`onkeychange` is available as a JS property, and also dispatches a same-named DOM custom
event (bubbles, composed) for a consumer who only calls `addEventListener` — the detail is
the callback's own key string:

```js
const tabs = document.querySelector('sui-tabs');
tabs.addEventListener('keychange', (e) => console.log(e.detail));
```
