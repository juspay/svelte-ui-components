...
| dropdownAlign    | `'left' \| 'right'`                                                      | No       | `'left'`       | **Deprecated — use `placement`.** Horizontal anchor of the dropdown panel. `'left'` anchors to the trigger's left edge; `'right'` anchors to the right edge so a wider-than-trigger panel hangs leftward without overflowing. Still honoured when `placement` is unset; removal in 5.0.0.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| placement        | `'bottom-left' \| 'bottom-right' \| 'top-left' \| 'top-right' \| 'auto'` | No       | `-`            | Corner the dropdown panel anchors to, matching `Menu`'s prop of the same name. Controls both axes, so an in-flow Select can open **upward** — which `dropdownAlign` could not express. `'auto'` resolves the best-fitting corner against the viewport on open. Unset keeps the previous behaviour exactly: in-flow panels open downward, portaled ones flip up only when they cannot fit below **and there is more room above than below** — a panel that fits nowhere stays below rather than flipping into a tighter space.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| usePortal        | `boolean`                  | No       | `false`        | When `true`, the dropdown panel is portaled to the root of the tree it lives in — `document.body` normally, or the shadow root inside `<sui-select>` — and positioned `fixed` relative to the trigger, so an ancestor with `overflow: hidden` or a scroll container (e.g. a table cell) cannot clip it. The root is chosen rather than always `document.body` because Svelte scopes a custom element's CSS to its shadow root: a panel moved into the light DOM keeps its markup and loses every rule scoped to that root, so through `<sui-select>` it would render unstyled. Placement follows the trigger on scroll/resize and flips above it when there's no room below *and* more room above. Portaled panels default to `z-index: 1000`; raise `--select-dropdown-z-index` to sit above an even higher overlay. |
| name             | `string`                                                                 | No       | -              | Sets the `name` under which selected ids submit as part of a surrounding `<form>` (`FormData`). Select renders no native `<select>`, so this renders one `<input type="hidden" {name} value={id}>` per selected id — the same shape a native `<select multiple>` produces: one entry per selection, none when nothing is selected. An empty string is treated exactly as omitted -- no hidden inputs are rendered, so `name=""` silently disables form participation rather than submitting under an empty key. This deliberately diverges from a native `<select name="">`, which does submit under an empty key: Select has no underlying native control to inherit that behaviour from, so an empty key would be a hidden input this component invented, not one the platform requires. Omitting it leaves the DOM byte-identical to before. |
...
`placement` anchors the panel to a corner of the trigger, on both axes. It matches
`Menu`'s prop of the same name, so the two components that open a panel from a
trigger are configured identically.

```svelte
<!-- Opens upward — for a Select sitting just above a sticky action bar. -->
<Select {items} bind:value placement="top-left" />

<!-- Picks the corner that fits, measured against the viewport on open. -->
<Select {items} bind:value placement="auto" />
```

Leaving `placement` unset keeps the previous behaviour exactly, which is what the
deprecated `dropdownAlign` still drives: an in-flow panel always opens downward,
and a portaled one flips up only when it **cannot fit below and there is more
room above**. A portaled panel that fits below neither side stays below, because
neither side is better. Setting `placement`
opts both paths into the same resolved corner — and it is the only way to make an
in-flow panel open upward, because `dropdownAlign` describes the horizontal axis
only.

`'auto'` renders the panel hidden for one frame to measure it before choosing a
corner, so it costs a tick that a pinned corner does not. Pin the corner when you
already know which way the panel should open.

The resolved corner is published on the panel as `data-placement`. It reports
where the panel actually went, not what was asked for — so a portaled panel that
flipped on fit under an unset `placement` reports `top-*`, and a consumer can
style the flipped state — flipping a drop shadow upward, for instance:
...
> **Web component note:** a portaled panel is appended to `document.body`, which is
> outside the shadow root, so a `:global([data-placement^='top'])` rule written
> inside the component's own styles will not reach it. An in-flow panel stays in
> the shadow tree and is styled normally. Scope the rule at the document level, or
> use `usePortal={false}`, when the flipped-state styling has to apply to both.

### Migrating from `dropdownAlign`

| Before                  | After                      |
| ----------------------- | -------------------------- |
| `dropdownAlign="left"`  | `placement="bottom-left"`  |
| `dropdownAlign="right"` | `placement="bottom-right"` |

`dropdownAlign` keeps working until 5.0.0. When both are set, `placement` wins.
...
<script>
  ...
  el.onchange = (value) => console.log(value);
</script>
```

All four events are available as JS properties, assigned as above. `onclear` and `onopen`
also dispatch a same-named DOM custom event (bubbles, composed, no detail) for a consumer
who only calls `addEventListener`:

```js
el.addEventListener('clear', () => console.log('selection cleared'));
el.addEventListener('open', () => console.log('dropdown opened'));
```

`onchange` and `onclose` do NOT dispatch a matching DOM event — `change` and `close` are
already `HTMLElement`'s own native events, so `el.addEventListener('change', ...)` or
`el.addEventListener('close', ...)` registers without error but never runs. Use the JS
property for those two instead: `el.onchange = (value) => ...`, `el.onclose = () => ...`.
