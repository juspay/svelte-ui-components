...
`onpagechange` is available as a JS property, and also dispatches a same-named DOM custom event
(bubbles, composed) for a consumer who only calls `addEventListener` — the detail is the new page
number, unchanged from the callback's own argument:

```js
const book = document.querySelector('sui-book');
