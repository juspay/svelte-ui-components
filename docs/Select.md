...
| usePortal        | `boolean`                                                                | No       | `false`        | When `true`, the dropdown panel is portaled to `document.body` and positioned `fixed` relative to the trigger, so an ancestor with `overflow: hidden` or a scroll container (e.g. a table cell) cannot clip it. Placement follows the trigger on scroll/resize and flips above it when there's no room below *and* more room above. Portaled panels default to `z-index: 1000`; raise `--select-dropdown-z-index` to sit above an even higher overlay.                                                                                                                                                                                                                                                                                                                                                                                                                |
