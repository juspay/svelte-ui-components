...
`onclick` is a callback prop, not a DOM event — assign it as a plain property
(`el.onclick = (checked) => ...`). The custom element does not dispatch a matching DOM
event: `click` is already `HTMLElement`'s own native event, so `el.addEventListener('click',
...)` still sees the real click that bubbles out of the shadow root on its own, and adding a
second, synthetic one under the same name would double-deliver it to that same listener.
