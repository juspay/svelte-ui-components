...
`onadd`, `ondismiss` and `onedit` are available as JS properties, and each also dispatches a
same-named DOM custom event (bubbles, composed) for a consumer who only calls
`addEventListener` — `add`'s and `dismiss`'s detail is the chip string as-is, `edit`'s is
`{ value, previousValue }`:

```js
const chipInput = document.querySelector('sui-chip-input');
chipInput.addEventListener('add', (e) => save(e.detail));
chipInput.addEventListener('edit', (e) => rename(e.detail.previousValue, e.detail.value));
```

`onchange` stays a JS-property-only callback and does not dispatch a DOM event: `change` is
already `HTMLElement`'s own native event, so a `chipInput.addEventListener('change', ...)`
