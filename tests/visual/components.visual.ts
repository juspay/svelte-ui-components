...
async function prepare(page: Page, slug: string): Promise<void> {
  ...
  //
  // `settleClock`, not a single `runFor`: firing the whole budget in one call
  // leaves a cascade wherever the real event loop's scheduler happens to be
  // under load (see its own doc comment), which is exactly the mid-cascade
  // state the paragraph above needs this call to avoid measuring.
