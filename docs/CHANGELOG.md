...
## [Unreleased](https://github.com/juspay/svelte-ui-components/compare/HEAD..3.1.6)

Phase 1 of the event-casing migration described in docs/EVENT_CASING_MIGRATION.md.
Each of the 142 grandfathered props gains its correct spelling alongside the
old one, both wired to the same handler. Nothing is removed, nothing is
renamed, and a consumer on an old spelling is untouched.

Generated by scripts/migrate/alias-event-props.ts, committed rather than run
once and discarded, because it is the only honest description of how 56
components were edited at the same time. Re-running it is a no-op, and a test
asserts that.

The transform is narrow on purpose. For `onclick` -&gt; `onClick` it rewrites
...
so every existing reference keeps resolving and no call site is touched. That
matters more than it sounds: Toggle.svelte both destructures a synthetic
`onclick` prop and renders `&lt;input onclick={handleCheckboxClick}&gt;`, and a
textual rename would have corrupted the second. Only the binding inside the
`$props()` pattern is rewritten, and an attribute name is not a binding.

Two problems the generator had, both caught before they shipped:

- It replaced the whole destructured property, which dropped defaults.
`onFocus = () =&gt; {}` became a bare binding, and Input calls
`onFocusout(event); onBlur(event)` unguarded -- every consumer not passing
those props would have hit "onBlur is not a function" on blur. Defaults now
travel onto the legacy binding, and a test pins that.
- Declarations are not reliably one line, and their types carry their own `;`
(`(event: { index: number; dataPoint: T }) =&gt; void`). The end of a
declaration is now found by scanning to the first `;` at depth zero.

casing.test.ts's collision guard is inverted by this change. It asserted the
corrected spellings were absent, which is what made the rename safe to plan and
is wrong once they exist. It now asserts each target is declared exactly once,
which is the property still worth holding -- a duplicate is what a re-run would
leave.

Verified: 404/404 Playwright functional specs across all 56 components,
295/295 unit, 143/143 across scripts/, lint and check clean. The event-casing
checker still reports 142 known and 0 new, because the added spellings are
correct by construction. The toggle demo now exercises both spellings, and a
spec asserts each reaches the handler.
