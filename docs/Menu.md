...
| usePortal        | `boolean`                                                                | No       | `false`         | Renders the dropdown into `document.body` so a clipping ancestor cannot cut it off, with placement following the trigger on scroll and resize. Defaults to false, which keeps the in-flow `position: absolute` behaviour and any consumer CSS reaching `.menu-dropdown` through an ancestor selector. Portaled panels default to `z-index: 1000`; raise `--menu-z-index` to clear a higher overlay. |
