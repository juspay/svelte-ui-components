...
A `compare` cell's `trendPercent` defaults to plain text; set `animateTrend: true` on that row's cell data to roll it through `AnimatedNumber` instead, so a trend that updates in place (a live dashboard, a polling total) counts to its new value rather than jumping. It's per-cell rather than a column- or table-level switch, so a column can mix live rows with historical ones that shouldn't move. The up/down arrow, its colour, and the flat (`trendPercent === 0`) case are unaffected. The `%` is passed through `AnimatedNumber` with the number rather than left beside it, so it forms part of the odometer's accessible name — a unit outside a `role="img"` element is never reached by graphic navigation — and it stays a literal column that never rolls. Passing the formatted string also means `trendPercent` renders exactly as the un-animated branch shows it, without `Intl` grouping a large value into `1,234.5`.
