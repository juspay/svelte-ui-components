...
optionally expandable to reveal the reasoning behind it. One component covers four shapes — a live
status row, a disclosure with a plain reasoning paragraph, a bare label for inside a chat bubble,
and a disclosure whose body is a kind-aware, host-streamed reasoning **trace** (checklist,
reasoning prose, search sources, or file edits).
...
| `variant`         | `'default' \| 'bare'`                            | No       | `'default'` | `bare` renders only the shimmering label — no avatar, no disclosure, never expandable.                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `showElapsed`     | `boolean`                                        | No       | `false`     | Live `Ns` counter while the label is busy; freezes once it settles. No effect on `bare`.                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `busy`            | `boolean`                                        | No       | `-`         | Host-owned turn state. While `true`: the label shimmers, the `steps` kind's newest row shows a live spinner, the elapsed counter ticks, and the disclosure auto-opens. Flipping to `false` freezes the counter, fires `onsettled` once, and — unless the user toggled the disclosure by hand — schedules the auto-collapse after `collapseDelayMs`. Omitted entirely: the label shimmers exactly per the original rule (settled while expandable, live otherwise) and no automatic disclosure machine runs. |
