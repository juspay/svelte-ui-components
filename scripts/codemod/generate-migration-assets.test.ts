...
describe('collectWcDisplayEntries — against this repo’s own real src/wc/components', () => {
  it('proves reachability: the real directory this build reads from yields the 98 rules the docs claim, with nothing skipped', () => {
    ...
    expect(new Set(entries.map((e) => e.tag)).size).toBe(98);
  });
});
