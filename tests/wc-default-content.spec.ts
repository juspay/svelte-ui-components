...
test.describe('custom-element props do not shadow the platform', () => {
  ...
  /**
   * `Checkbox` declared `text` mandatory and dereferenced `text.length`. The
   * Svelte build is type-checked so no caller could omit it; the custom-element
   * build type-checks nothing, so `<sui-checkbox name="terms">` threw during
   * render and left an empty shadow root -- silently, because a custom element
   * that fails to render looks exactly like one that has not upgraded yet.
   *
   * Found while proving that the form value is never read from a stale host
   * prop: the fallback path that reads host props is only reachable when the
   * component renders no native control, and this was making that true.
   */
  test('sui-checkbox renders with no text attribute at all', async ({ page }) => {
    const errors: string[] = [];
    page.on('pageerror', (error) => errors.push(error.message));
    ...
    const measured = await page.evaluate(async () => {
      ...
      host.innerHTML = `<sui-checkbox name="terms"></sui-checkbox>`;
      document.body.append(host);
      await new Promise((resolve) => setTimeout(resolve, 300));
      const box = host.firstElementChild;
      const root = box?.shadowRoot ?? null;
      return {
        rendered: (root?.innerHTML ?? '').length > 0,
        hasNativeInput: root?.querySelector('input') !== null && root !== null,
        role: root?.querySelector('[role="checkbox"]') !== null
      };
    });

    expect(errors, 'render must not throw').toEqual([]);
    expect(measured.rendered).toBe(true);
    expect(measured.hasNativeInput).toBe(true);
    expect(measured.role).toBe(true);
  });
});
