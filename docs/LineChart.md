...
the default, shows nothing — the legend renders exactly as before. Matching the gap-point behavior
above, every kind, `sum` included, skips non-finite `y` values rather than treating them as zero
...
<LineChart
  series={[
    { name: 'Product A', data: [...], aggregate: 'sum', aggregateFormat: (v) => `${v} units` },
    { name: 'Product B', data: [...], aggregate: 'average' }
  ]}
  showLegend
/>
```
...
| ----------------- | ----------------------------------------------------------- | -------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| series            | `LineChartSeries[]`                                         | Yes      | `-`          | Array of `{name, data, color?, aggregate?, aggregateFormat?}`. Each series renders as a separate line. `aggregate` (`'sum' \| 'average' \| 'min' \| 'max' \| 'none'`, default `'none'`) shows a computed value beside the legend label; `aggregateFormat` defaults to `yTickFormat`. See "Legend Aggregates" below. |
