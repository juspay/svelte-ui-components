...
| ariaLabel            | `string \| null`                                                       | No       | `-`                 | Accessible name applied as `aria-label` on the underlying `<input>`/`<textarea>`. Use whenever the field has no visible `label` (e.g. inline table cells, icon-only search fields).                                           |
