...
`onleftimageclick`, `onrightimageclick`, `oncentertextclick`, `onitemclick`, and
`ontopsectionclick` are available as JS properties, and each also dispatches a same-named DOM
custom event (bubbles, composed, detail: the `MouseEvent` argument) for a consumer who only calls
`addEventListener`:

```js
const item = document.querySelector('sui-list-item');
item.addEventListener('itemclick', (e) => console.log(e.detail));
item.addEventListener('leftimageclick', (e) => console.log(e.detail));
```

`onkeydown` is a JS-property callback only and does not dispatch a DOM event: it is already a
...
Under `prefers-reduced-motion: reduce` the continuous fill becomes **five discrete jumps** rather than being removed outright: a bare "stop" would freeze the bar at its base `width: 100%` and read as finished for the whole duration. Stepping holds each interval's PRIOR value, so the bar can under-report elapsed time but never overstate it. The step count is deliberately **not** a token — an invalid `steps()` value drops the declaration and restores the continuous slide, so a tunable would let a consumer switch the guard off by accident.
