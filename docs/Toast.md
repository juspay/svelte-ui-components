...
| direction            | `ToastDirection = 'left-to-right' \| 'right-to-left' \| 'top-to-bottom' \| 'bottom-to-top'` | No       | `-`     | The direction from which the toast enters and exits. Controls which axis and sign the CSS transition (`--toast-enter-x`/`--toast-enter-y`) travels along.                                                                                                                                                                                                                  |
...
| inAnimationOffset    | `number \| null`                                                                            | No       | `-`     | Pixel offset for the toast's closed position (`--toast-enter-x`/`--toast-enter-y`) — the single position CSS transitions both from (entrance) and back to (exit), since a `@starting-style` rule can't hold an independent value from the non-visible state. Takes priority over `outAnimationOffset` when both are set. Higher values mean the toast starts further away. |
| inAnimationDuration  | `number \| null`                                                                            | No       | `-`     | Duration in milliseconds for the entrance transition (opacity/transform/display). Independent of `outAnimationDuration`.                                                                                                                                                                                                                                                   |
| outAnimationOffset   | `number \| null`                                                                            | No       | `-`     | Pixel offset for the toast's closed position. Only takes effect when `inAnimationOffset` is unset — see `inAnimationOffset`, which wins when both are set and applies to entrance and exit alike. Higher values mean the toast exits further away.                                                                                                                         |
| outAnimationDuration | `number \| null`                                                                            | No       | `-`     | Duration in milliseconds for the exit transition (opacity/transform/display). Independent of `inAnimationDuration`.                                                                                                                                                                                                                                                        |
...
| `--toast-open-duration`                | `var(--duration-slow, var(--motion-duration, 0.4s))` | transition-duration        | Duration of the entrance transition (opacity, transform, display). Set from the `inAnimationDuration` prop.                                                                                                                                                                             |
| `--toast-close-duration`               | `var(--motion-duration, 0.8s)`                       | transition-duration        | Duration of the exit transition (opacity, transform, display). Set from the `outAnimationDuration` prop; floored at 50ms so the `transitionend` listener that fires `ontoasthide` always runs.                                                                                          |
| `--toast-open-easing`                  | `var(--ease-out, var(--motion-easing, ease-out))`    | transition-timing-function | Timing function for the entrance transition.                                                                                                                                                                                                                                            |
| `--toast-close-easing`                 | `var(--motion-easing, ease-in)`                      | transition-timing-function | Timing function for the exit transition.                                                                                                                                                                                                                                                |
| `--toast-enter-x`                      | `0px` (direction-dependent — see description)        | transform                  | Horizontal offset of the toast's closed position — where it enters from and exits to. Set per `direction` (`left-to-right`/`right-to-left`) by default; overridden by the `inAnimationOffset`/`outAnimationOffset` props.                                                               |
| `--toast-enter-y`                      | `0px` (direction-dependent — see description)        | transform                  | Vertical offset of the toast's closed position — where it enters from and exits to. Set per `direction` (`top-to-bottom`/`bottom-to-top`) by default; overridden by the `inAnimationOffset`/`outAnimationOffset` props.                                                                 |
