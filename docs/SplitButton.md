...
`onclick` and `onselect` are JS-property callbacks only (`el.onclick = (event) => ...`,
`el.onselect = (item) => ...`) and do NOT dispatch a matching DOM event: `click` and
`select` are already `HTMLElement`'s own native events, so
`el.addEventListener('click', ...)` still sees only the real, natively-bubbled click, not
a synthetic one, and `el.addEventListener('select', ...)` never runs for `onselect`.
