...
<script lang="ts">
  ...
  // 'sui-avatar:onclick' in ../dispatch.ts's DISPATCH_COLLISION_EXCEPTIONS -- so
  // dispatchEvents intentionally returns nothing for it. It stays callback-only:
  // `avatar.onclick = fn` still runs `fn(event)` exactly as before, but no synthetic
  ...
  // prevent). Called anyway, rather than special-cased away: proves the collision
...
<style>
  ...
  :host {
    display: var(--sui-avatar-display, inline-block);
  }
</style>
