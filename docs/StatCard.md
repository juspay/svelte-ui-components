...
`rows[]` entries accept the following, in addition to `heading` and `value`:

| Field                    | Type                      | Description                                                                                                                                                      |
| ------------------------ | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `subtitle`               | `string`                  | Secondary label below this row's value line. Independent of the card-level `subtitle`; both may be set, e.g. when rows compare against different baselines.      |
| `additionalContent`      | `string`                  | Trailing text on the value line (a unit, a caption).                                                                                                             |
| `additionalContentBreak` | `boolean`                 | Forces `additionalContent` onto its own line below the value and delta. Without it the break is incidental to the container width, because the value line wraps. |
| `valueVariant`           | `'success' \| 'warning'`  | Tints this row's value text. Reads `--statcard-value-success-color` / `--statcard-value-warning-color` — see the note under CSS Variables.                       |
| `comparisonValue`        | `string`                  | Inline denominator, e.g. `/ ₹10L`.                                                                                                                               |
| `change`                 | `number \| null`          | `number` renders the delta (`—` at 0); `null` renders `N/A`; omit to render nothing.                                                                             |
| `invertChangeColors`     | `boolean`                 | Swaps the positive/negative colouring, for metrics where down is good.                                                                                           |
| `tooltip`                | `StatCardTooltip`         | Tooltip on the row heading — `{ text, position?, testId? }`.                                                                                                                                      |
| `breakdownHeading`       | `string`                  | Heading above the breakdown grid.                                                                                                                                |
| `breakdown`              | `StatCardBreakdownItem[]` | Grid of sub-metrics under the row.                                                                                                                               |
| `testId`                 | `string`                  | Renders as `data-pw`/`testID` on the row root. Also the CSS scoping anchor for per-row value/heading typography — see the note under CSS Variables.              |
...
| `--statcard-row-heading-font-size`         | `12px`                                 | font-size      | Font size of a row heading.                                                                                                                         |
| `--statcard-row-heading-font-weight`       | `500`                                  | font-weight    | Font weight of a row heading.                                                                                                                       |
| `--statcard-row-heading-color`             | `#6b7280`                              | color          | Colour of a row heading.                                                                                                                            |
| `--statcard-row-heading-line-height`       | `1.4`                                  | line-height    | Line height of a row heading.                                                                                                                       |
| `--statcard-row-heading-order`             | `0`                                    | order          | Flex order of a row's heading wrap among its own sub-elements (heading, value line, subtitle).                                                      |
| `--statcard-row-value-line-order`          | `0`                                    | order          | Flex order of a row's value line among its own sub-elements.                                                                                        |
| `--statcard-row-subtitle-order`            | `0`                                    | order          | Flex order of a row's subtitle among its own sub-elements.                                                                                          |
...
| `--statcard-value-success-color`           | `#16a34a`                              | color          | Value text colour when a row sets `valueVariant="success"`.                                                                                         |
| `--statcard-value-warning-color`           | `#f59e0b`                              | color          | Value text colour when a row sets `valueVariant="warning"`.                                                                                         |
| `--statcard-row-subtitle-font-size`        | `--statcard-subtitle-font-size`        | font-size      | Font size of a per-row subtitle; falls back to the card-level subtitle size.                                                                        |
| `--statcard-row-subtitle-font-weight`      | `--statcard-subtitle-font-weight`      | font-weight    | Font weight of a per-row subtitle.                                                                                                                  |
| `--statcard-row-subtitle-color`            | `--statcard-subtitle-color`            | color          | Colour of a per-row subtitle.                                                                                                                       |
| `--statcard-row-subtitle-line-height`      | `--statcard-subtitle-line-height`      | line-height    | Line height of a per-row subtitle.                                                                                                                  |
| `--statcard-row-value-font-size`           | `--statcard-value-font-size`           | font-size      | Font size of a single row's value; falls back to the card-level value size. Scoped to rows only — the single (non-`rows`) card value is unaffected. |
| `--statcard-row-value-font-weight`         | `--statcard-value-font-weight`         | font-weight    | Font weight of a single row's value; falls back to the card-level value weight.                                                                     |
| `--statcard-row-additional-flex-basis`     | `100%`                                 | flex-basis     | Width the additional content takes when `additionalContentBreak` is set.                                                                            |

> **`valueVariant` needs two tokens defined by the adopting app.** Its defaults are this
> library's own green and amber, deliberately literal so the component renders correctly
> with no stylesheet at all. An app with its own semantic success and warning colours should
> point `--statcard-value-success-color` and `--statcard-value-warning-color` at them —
> otherwise the variant silently renders the library's colours, in both themes, with nothing
> failing.

> **Per-row typography is a CSS custom property, scoped through the row's own `testId`.**
> `--statcard-row-value-font-size`/`--statcard-row-value-font-weight` and
> `--statcard-row-heading-font-size`/`--statcard-row-heading-font-weight` are ordinary CSS
> custom properties that inherit down from whatever element sets them — there is no
> `StatCardRow` field for the value itself. To make one row's value or heading render at a
> different size/weight than the rest of the card, give that row a `testId` and target it
> (the root `.statcard-row` element carries `data-pw="<row.testId>"`):
>
> ```css
> [data-pw='primary-kpi-row'] {
...
> ```
>
> A row that does not set a matching selector falls back to the card-level value tokens
> (`--statcard-value-font-size`/`--statcard-value-font-weight`) exactly as before this pair of
> variables existed — an existing card with no such CSS renders byte-identically.

> **Per-row order is a CSS custom property too, scoped the same way.** A row's heading wrap,
> value line, and subtitle render in that markup order by default — `--statcard-row-heading-order`,
> `--statcard-row-value-line-order`, and `--statcard-row-subtitle-order` all default to `0`, so
> ties resolve to markup order and an untouched row is byte-identical to today. Set them on the
> same `[data-pw="<row.testId>"]` selector to rearrange a single row's sub-elements — e.g. title →
> subtitle → value:
>
> ```css
> [data-pw='primary-kpi-row'] {
...
> ```
>
> This mirrors the card-level `--statcard-header-order`/`--statcard-subtitle-order`/
> `--statcard-value-row-order` hooks, which reorder the card's own top-level sections the same
> way. `.statcard-row` is already a `display: flex; flex-direction: column` container, so `order`
> applies with no other CSS needed.
