...
- Child elements are sanitized before adoption, not inlined verbatim: `<script>` and `<foreignObject>` subtrees are removed outright, every `on*` attribute is stripped at any depth, and `href`/`xlink:href`/`src` values pointing at `javascript:` are dropped. Without this, the root allowlist alone would be bypassable — the root is not where a hostile file would put its payload. Everything else about a child (its geometry, styling, `id`, `class`) survives untouched.
