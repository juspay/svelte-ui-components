...
`onretry` is available as a JS property, and also dispatches a same-named DOM custom event
(bubbles, composed) for a consumer who only calls `addEventListener` — the detail is the
callback's own row index:

```js
const list = document.querySelector('sui-task-list');
list.addEventListener('retry', (e) => console.log('retry', e.detail));
```
